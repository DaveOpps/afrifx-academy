...
export interface Tick { sym: string; display: string; price: number; change: number; dp: number; }
export interface Candle { t: number; o: number; h: number; l: number; c: number; v: number; }
export interface DepthLevel { price: number; qty: number; }
export interface Depth { bids: DepthLevel[]; asks: DepthLevel[]; }
export interface Trade { price: number; qty: number; buyerMaker: boolean; time: number; }
...
export async function fetchTickers(): Promise<Tick[] | null> {
  try {
...
export async function fetchTicker24h(symbol: string): Promise<any | null> {
  try {
...
export async function fetchKlines(symbol = 'BTCUSDT', interval = '1h', limit = 60): Promise<Candle[] | null> {
  try {
...
export async function fetchTrades(symbol = 'BTCUSDT', limit = 24): Promise<Trade[] | null> {
  try {
    const res = await fetch(`${BASE}/trades?symbol=${symbol}&limit=${limit}`);
    if (!res.ok) return null;
    const data = await res.json();
    return data.map((t: any) => ({ price: +t.price, qty: +t.qty, buyerMaker: t.isBuyerMaker, time: t.time })).reverse();
  } catch { return null; }
}

export async function fetchDepth(symbol = 'BTCUSDT', limit = 12): Promise<Depth | null> {
  try {
